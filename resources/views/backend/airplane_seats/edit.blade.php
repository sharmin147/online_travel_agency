...
<div class="row">
    <div class="col-sm-12">
        <form action="{{ route('airplane_seats.update', $airplaneSeat->id) }}" method="post">
            @csrf
            @method('PATCH')
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select id="airplane_id" class="form-control" name="airplane_id">
                            ...
                            @forelse($airplane as $ap)
                                <option value="{{$ap->id}}" @if(old('airplane_id',$airplaneSeat->airplane_id)==$ap->id) selected @endif>{{$ap->name}}</option>
                            @empty
                            @endforelse
                        </select>
                ...
                <div class="col-md-4 col-12">
                    <div class="form-group">
                        ...
                        <select id="flight_class_id" class="form-control" name="flight_class_id">
                            ...
                            @forelse($class as $ap)
                                <option value="{{$ap->id}}" @if(old('flight_class_id',$airplaneSeat->flight_class_id)==$ap->id) selected @endif>{{$ap->name}}</option>
                            @empty
                            @endforelse
                        </select>
                ...
                <div class="col-md-4 col-12">
                    <div class="form-group">
                        ...
                        <input type="quantity" name="quantity" id="quantity" value="{{$airplaneSeat->quantity}}" class="form-control">
                    </div>
                </div>
            </div>
