...
<div class="card">
    <div class="card-content">
        <div class="card-body">
            <form class="form" method="post" action="{{route('flight_route.update',$flightRoute->id)}}">
                @csrf
                @method('PATCH')
                <input type="hidden" name="uptoken" value="{{encryptor('encrypt',$flightRoute->id)}}">
                <div class="row">
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            <label for="name" style="color: green;">Route Name</label>
                            <input type="string" id="name" class="form-control" value="{{ old('name',$flightRoute->name)}}" name="name">
                        </div>
                    </div>
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            ...
                            <select id="departure_city" onchange="checkairport(this)" class="form-control" name="departure_city">
                                ...
                                @forelse($city as $c)
                                    <option class="dc dc{{$c->id}}" value="{{$c->id}}" @if(old('departure_city',$flightRoute->departure_city)==$c->id) selected @endif>{{$c->name}}</option>
                                @empty
                                @endforelse
                            </select>
                    ...
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            ...
                            <select id="arrival_city" class="form-control" onchange="checkairport(this)" name="arrival_city">
                                <option value="">Select City</option>
                                @forelse($city as $c)
                                    <option class="ac ac{{$c->id}}" value="{{$c->id}}" @if(old('arrival_city',$flightRoute->arrival_city)==$c->id) selected @endif>{{$c->name}}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="departure_airport" class="form-control" name="departure_airport">
                                ...
                                @forelse($airport as $c)
                                    <option class="dcity dcity{{$c->city_id}}" value="{{$c->id}}" @if(old('departure_airport',$flightRoute->departure_airport)==$c->id) selected @endif>{{$c->name}}</option>
                                @empty
                                @endforelse
                            </select>
                    ...
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            ...
                            <select id="arrival_airport" class="form-control" name="arrival_airport">
                                <option value="">Select Airport</option>
                                @forelse($airport as $c)
                                    <option class="acity acity{{$c->city_id}}" value="{{$c->id}}" @if(old('arrival_airport',$flightRoute->arrival_airport)==$c->id) selected @endif>{{$c->name}}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
